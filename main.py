...
def train():
    ...
    plot_samples_2d(sample_batch, path=f'log/{dataset_name}/plots/', name='dataset')
    ...
    checkpoint_directory = "log/{}/tmp_{}".format(dataset_name, str(hex(random.getrandbits(32))))
    ...
    # start training
    for i in tqdm(range(max_epochs), desc='Epochs'):
        ...
        if i % int(1000) == 0:
            ...
            plot_heatmap_2d(maf, -4.0, 4.0, -4.0, 4.0, mesh_count=200, path=f'log/{dataset_name}/plots/heatmap/', name=f'epoch_{i}')
    ...
    plot_heatmap_2d(maf, -4.0, 4.0, -4.0, 4.0, mesh_count=200, path=f'log/{dataset_name}/plots/', name='evaluate1') 

    # plot samples of the best model
    plot_samples_2d(maf.sample(1000), path=f'log/{dataset_name}/plots/', name='evaluate2') 
